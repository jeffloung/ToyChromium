...
namespace STLib
{
	public static class IniHelper
	{
        ...
        public static Dictionary<string, string> ReadKeyValues(string section, string filePath)
        {
            while (true)
            {
                IntPtr returnedString = Marshal.AllocCoTaskMem(capacity * sizeof(char));
                ...
                if (size < capacity - 2)
                {
                    ...
                    foreach (var cmdItem in keyValuePairs)
                    {
                        dic.Add(cmdItem.Split('=')[0], cmdItem.Split('=')[1]);
                    }
                    return dic;
                }
